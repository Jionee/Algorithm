...
cand_chicken = list(combinations(chicken,M))
#print("치킨집후보 : ",cand_chicken)

sum_min = float('inf')
for cand in cand_chicken: #치킨집 후보들을 골라서 치킨거리 최소 구하기
    sum = 0
    for h in house: #각 집에서 치킨집까지의 치킨거리 구하기
        minChicken = float('inf')
        for c in cand:
            minChicken = min(calculate(h,c),minChicken) #계산된 거리 중 최소 값
        sum += minChicken #치킨거리 계산
